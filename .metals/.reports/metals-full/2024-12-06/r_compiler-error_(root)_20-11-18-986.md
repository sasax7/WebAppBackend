...
class StrategyController @Inject() (
  ...
  def getStrategiesByFirebaseUid(firebaseUid: String): Action[AnyContent] =
    ...
    def addStrategy(firebaseUid: String): Action[JsValue] = Action.async(parse.json) { request =>
  (request.body \ "name").validate[String].fold(
    erro@@rs => Future.successful(BadRequest(Json.obj("status" -> "error", "message" -> "Invalid JSON: Missing 'name' field"))),
    name => {
      strategyRepository.addStrategy(firebaseUid, name).map {
        case Some(strategyId) =>
          Created(Json.obj("status" -> "success", "strategyId" -> strategyId))
        case None =>
          NotFound(Json.obj("status" -> "error", "message" -> s"User with firebaseUid $firebaseUid not found"))
      }
    }
  )
}
