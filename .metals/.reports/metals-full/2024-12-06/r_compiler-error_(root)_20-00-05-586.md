...
class StrategyRepository @Inject() (
  ...
  def getStrategiesWithDetailsByFirebaseUid(
    ...
    val strategiesWithDetails = query.result.flatMap { strategiesAndUsers =>
      val strategyIds = strategiesAndUsers.map(_._1.id.get)

      val indicatorsQuery = strategyIndicators
      ...
      val stopLossesQuery = strategyStopLosses
        .filter(_.strategyId inSet strategyIds)
        .join(stopLossNames)
      ...
      val takeProfitsQuery = strategyTakeProfits
        .filter(_.strategyId inSet strategyIds)
      ...
      val pricePointsQuery = strategyPricePoints
        .filter(_.strategyId inSet strategyIds)
      ...
      val combinedQuery = for {
        ...
        stopLosses <- stopLossesQuery
        takeProfits <- takeProfitsQuery
        pricePoints <- pricePointsQuery
      } yield (indicators, stopLosses, takeProfits, pricePoints)

      db.run(combinedQuery).map {
        case (indicators, stopLosses, takeProfits, pricePoints) =>
          strategiesAndUsers.map { case (strategy, user) =>
            StrategyDetails(
              ...
              stopLossNames = stopLosses.collect {
                case (strategyId, name) if strategyId == stra@@tegy.id.get => name
              },
