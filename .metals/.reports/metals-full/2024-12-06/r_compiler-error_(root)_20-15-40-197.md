...
class StrategyRepository @Inject() (
  ...
  d@@ef addStrategy(firebaseUid: String, name: String): Future[Option[Long]] = {
    val action = for {
      // Find user by firebaseUid
      user <- users.filter(_.firebaseUid === firebaseUid).result.headOption
      strategyId <- user match {
        case Some(u) =>
          // Insert the new strategy
          (strategies returning strategies.map(_.id)) += Strategy(None, name)
            .flatMap { strategyId =>
              // Link the user to the new strategy
              userStrategies += UserStrategy(u.id.get, strategyId)
            }
            .map(Some(_))
