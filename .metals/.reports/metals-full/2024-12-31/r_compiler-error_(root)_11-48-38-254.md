...
== Source file context for tree position ==
    ...
    32   }
    33 
    34   def findStrategyDetailsById(id: Long): Future[Seq[StrategyDetails]] = {
    35     val query = for {
    36       (s, us) <- strategies join userStrategies on (_.id === _.strategyId)
    37       u <- users if u.id === us.userId && u.firebaseUid === i_CURSOR_d
    38     } yield s
...
class StrategyRepository @Inject() (
  ...
  def findStrategyDetailsById(id: Long): Future[Seq[StrategyDetails]] = {
    val query = for {
      (s, us) <- strategies join userStrategies on (_.id === _.strategyId)
      u <- users if u.id === us.userId && u.firebaseUid === i@@d
    } yield s
