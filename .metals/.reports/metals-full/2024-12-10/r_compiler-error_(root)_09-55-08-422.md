...
class CandlesRepository @Inject() (
  ...
  val actions = timeframes.flatMap { timeframe =>
    ...
    oneMinCandlesticks
      .groupBy(c => (c.pair_id, ((c.time - baseTimeReference) / duration) * duration + baseTimeReference))
      .map { case ((pairId, aggregatedTime), candles) =>
        // Debug: Log grouping details for the timeframe
        println(s"Aggregating for timeframe $timeframe: Pair $pairId, Time $aggregatedTime, Candles Count: ${candles.size}")
        ...
        val upsertAction = existingCandleQuery.result.headOption.flatMap {
          case Some(_) =>
            ...
            existingCandleQuery.update(aggregatedCandle)
          case None =>
            // Insert new candle
  ...
  db.run(DBI@@O.sequence(actions).transactionally).map(_ => ())
}
