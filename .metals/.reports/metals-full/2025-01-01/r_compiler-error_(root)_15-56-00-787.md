file:///C:/Users/sti/OneDrive%20-%20Leicom%20AG/Desktop/HTWG/Web_App/backend/app/controllers/BacktestController.scala
...
class BacktestController @Inject() (
  ...
  private var context: BacktestContext = _
    def addStopLoss: Action[AnyContent] = Action.async { implicit request =>
    val json = request.body.asJson.get
    val price = (json \ "price").as[Double]
    val tradeId = (json \ "tradeId").asOpt[Long]
    val stopLossNameId = (json \ "stopLossNameId").as[Long]

    // Assuming you have a method to get the current OperationalState
    val state: OperationalState = getCurrentOperationalState()

    state.addStopLoss(price, tradeId, stopLossNameId).map { updatedState =>
      Ok(Json.toJson(updatedState))
    }
  }

  def addTakeProfit: Action[AnyContent] = Action.async { implicit request =>
    val json = request.body.asJson.get
    val price = (json \ "price").as[Double]
    val tradeId = (json \ "tradeId").asOpt[Long]
    ...
    state.addTakeProfit(price, tradeId, takeProfitNameId).map { updatedState =>
      Ok(Json.toJson(updatedState))
    }
  }

  def addIndicator: Action[AnyContent] = Action.async { implicit request =>
    val json = request.body.asJson.get
    ...
    state.addIndicator(value, time, indicatorNameId, tradeId).map { updatedState =>
      Ok(Json.toJson(updatedState))
    }
  }

  def addPricePoint: Action[AnyContent] = Action.async { implicit request =>
    val json = request.body.asJson.get
    val value = (json \ "value").as[Double]
    ...
    state.addPricePoint(value, time, pricePointNameId, tradeId).map { updatedState =>
      Ok(Json.toJson(updatedState))
    }
  }
