...
case class OperationalState(
   ...
   def addIndicator(
    ...
    idToUse match {
      case Some(id) =>
        advancedTradeRepository.addIndicator(value, Some(timeToUse), indicatorNameId, id).map { indicator =>
          val updatedTrades = trades.map { trade =>
            if (trade.addAdvancedTradeObject.exists(_.trade.id.contains(id))) {
              val updatedTradeObject = trade.addAdvancedTradeObject.map { ato =>
