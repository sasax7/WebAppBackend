  ...
  val action = for {
    ...
    effectiveEntryPrice <- triggeredDateOpt match {
      case Some(triggeredTime) if triggeredTime == start_date.getTime =>
        getNextCandleOpen(triggeredTime).map(_.getOrElse(entry_price.toDouble))
      case _ =>
    ...
    highestRR = {
      val profitDistance = math.abs(highestPrice - effectiveEntryPrice)
      val riskDistance = math.abs(effectiveEntryPrice - stop_loss.toDouble)
      if (riskDistance > 0) profitDistance / riskDistance else 0.0
    }

    // Insert the AdvancedTrade and retrieve its new ID in one step
    newTradeId <- (advancedTrades returning advancedTrades.map(_.id)) += AdvancedTrade(
      id = None,
    ...
    // Insert the StopLoss for the new trade
    ...
    // Finally, fetch and return the newly created AdvancedTrade row
    insertedTrade <- advancedTrades.filter(_.id === newTradeId).result.head
  } yield insertedTrade
