...
case class OperationalState(
  ...
  override def addTra@@de(tradeData: AddTradeRequest)(implicit
      ec: ExecutionContext,
    ...
    (for {
      sId <- strategy.flatMap(_.id) // Extract the Long from Option[Long]
      pId <- currentPair.id // Same for the pair
    } yield {
      advancedTradeRepository
        .addAdvancedTrade(
          ...
          strategy_id = sId, // Now sId is a Long
          pair_id = pId // pId is also a Long
        )
        .map {
          case Some(_) =>
            // If successfully added, just return this (or update further if needed).
            this
          case None =>
            this // If the trade couldn't be added, do nothing more.
        }
    }).getOrElse(
      Future.successful(this)
    ) // Fallback if strategy or pair is None
  }
}
