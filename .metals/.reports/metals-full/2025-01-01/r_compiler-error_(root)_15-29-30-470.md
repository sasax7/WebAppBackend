...
class AdvancedTradeRepository @Inject() (
  ...
  def addTakeProfit(
    ...
    val action = for {
      ...
      _ <- calculateRRs(tradeId, newTakeProfit.id.get, price, isBuy, entryPrice)
    } yield newTakeProfit
  ...
  val entryPriceQuery = advancedTrades.filter(_.id === tradeId).map(_.entryPrice).result.head
  ...
  def highestPriceQuery(startTime: Long, endTime: Long, isBuy: Boolean): DBIO[Double] = {
    val query = candlesticks
      .filter(c =>
        c.pair_id === pairId &&
          c.timeframe === "1m" &&
  ...
  val action = for {
    ...
    newStopLoss <- (stopLosses returning stopLosses.map(_.id) into ((stopLoss, id) =>
      stopLoss.copy(id = Some(id))
    )) += StopLoss(
      id = None,
      price = price,
      timeHit = timeHitOpt.map(new Timestamp(_)),
    ...
    _ <- calculateRRs(tradeId, newStopLoss.id.get, price, isBuy, entryPrice)
  } yield newStopLoss
