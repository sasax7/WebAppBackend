...
case class OperationalState(
  ...
  def addIndicator(
    ...
    idToUse match {
      case Some(id) =>
        advancedTradeRepository.addIndicator(value, time, indicatorNameId, id)
      case None =>
        Future.failed(new IllegalArgumentException("No tradeId provided and no latestTradeId available"))
    }
  }
  def addTradeAsync(
