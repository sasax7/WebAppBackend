...
def addTradeAsync(
    ...
    (for {
      ...
      advancedTradeRepository
        ...
        .map { advTrade =>
          val newTradeObj = AdvancedTradeObject(
            trade = advTrade,
            stopLosses = Seq.empty,
            takeProfits = Seq.empty,
            indicators = Seq.empty,
            pricePoints = Seq.empty,
            tradeRRs = Seq.empty
          )
          val newTrade = convertToTrade(newTradeObj)(
            state.newestTimestamp,
            state.balance,
            state.percentageRiskP@@erTrade,
            state.currentPrice
          )
          state.copy(trades = state.trades :+ newTrade)
        }
...
object OperationalState {
  ...
  def addTrade(tradeData: AddTradeRequest)(implicit
      ec: ExecutionContext,
      advancedTradeRepository: AdvancedTradeRepository,
      newestTimestamp: Long,
      strategy: Option[Strategy],
    ...
    (for {
      ...
      advancedTradeRepository
        .addAdvancedTrade(
          start_date = new Timestamp(newestTimestamp),
          entry_price = tradeData.entryPrice,
          stop_loss = tradeData.stopLoss,
        ...
        .map { advTrade =>
          // Build a minimal AdvancedTradeObject (we already have the inserted AdvancedTrade)
          val newAdvancedTradeObject = AdvancedTradeObject(
            trade = advTrade,
            stopLosses = Seq.empty,
            takeProfits = Seq.empty,
            indicators = Seq.empty,
          ...
          // Convert it to our local Trade structure
          val newTrade = convertToTrade(newAdvancedTradeObject)(
            newestTimestamp,
            balance,
            percentageRiskPerTrade,
            currentPrice
          ...
          // Append it to the existing trades
          copy(trades = trades :+ newTrade)
        }
