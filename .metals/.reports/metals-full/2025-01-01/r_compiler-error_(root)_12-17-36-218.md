...
import models._
import repositories.{CandlesRepository, UsersRepository, StrategyRepository, AdvancedTradeRepository, PairsRepository}
      ...
      candlesticks.headOption match {
        case Some(latestCandle) =>
          ...
          copy(
            ...
            currentCandlestick = Some(latestCandle), // Update the current candlestick
...
object OperationalState {
  ...
  def initialize(
    ...
    for {
      ...
      strategyDetailsOpt <- strategyRepository.findStrategyDetailsById(strategyId)
      ...
      candlestickOpt <- candlesRepository
        .getCandlesticksBatchByPairName(currentPairName, "1m", newestTimestamp, 1)
        .map(_.headOption)
      advancedTrades <- advancedTradeRepository.getTradesByStrategyIdObject(strategyId)
    } yield {
      ...
      (userOpt, strategyOpt, strategyDetailsOpt, pairOpt, candlestickOpt) match {
        case (
              Some(user),
              Some(strategy),
        ...
        case _ =>
          throw new Exception("Invalid user, strategy, pair, or candlestick data")
      }
    }
  }
