...
class StrategyRepository @Inject() (
  ...
  d@@ef findStrategyDetailsById(id: Long): Future[Option[StrategyDetails]] = {
    val query = for {
    ...
    val strategiesWithDetailsAction = for {
      ...
      indicators <- strategyIndicators
        ...
        .map { case (si, i) => (si.strategyId, (i.id.get, i.name)) }
        .result
      ...
      stopLosses <- strategyStopLosses
        ...
        .map { case (ssl, sl) => (ssl.strategyId, (sl.id.get, sl.name)) }
        .result
      ...
      takeProfits <- strategyTakeProfits
        ...
        .map { case (stp, tp) => (stp.strategyId, (tp.id.get, tp.name)) }
        .result
      ...
      pricePoints <- strategyPricePoints
        ...
        .map { case (spp, pp) => (spp.strategyId, (pp.id.get, pp.name)) }
        .result
