...
import models.{Trade, Strategy, StrategyDetails, User, Candlestick}
  ...
  def initializeState: Action[JsValue] = Action(parse.json).async {
    implicit request =>
      val stateResult = for {
        ...
        currentPair <- (request.body \ "currentPair").asOpt[String]
  ...
  def updateTimestamp(newTimestamp: Long): Action[AnyContent] = Action.async {
    implicit request =>
      if (context == null) {
        ...
        context.updateTimestamp(newTimestamp).map { _ =>
          val currentState =
          ...
          Ok(
            Json.obj(
              ...
              "currentProfit" -> currentState.trades.headOption
                .flatMap(_.currentProfit),
              "currentCandlestick" -> currentState.currentCandlestick
            )
          )
        }
      }
